...
sphere = ball; //Possible because all properties of the sphere is present in the ball
ball = sphere; //Possible because all properties of the sphere is present in the ball
...
//The rationale(means reason) for the below two errors is that since the fresh types of the object literals are
...
//Where index signature can be defined as a function(for only ease) that is used to create dynamic properties in an object

var x: { id: number; [x: string]: any }; //Note now 'x' can have any name, just that the property should be of type string
// Property name should be a string but its value can be of any type
// If we change the value's type form "any" to "string" typescript will throw an error
//because it will also check if the value of property ID is of type string or not.
